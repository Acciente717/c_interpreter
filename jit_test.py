...
TEST_CASE_DIR = 'jit_cases/'
GROUNDTRUTH_DIR = 'jit_groundtruth/'
CACHE_FOLDER_NAME = '__cint_cache__'

all_code_file = os.listdir(TEST_CASE_DIR)
all_groundtruth_file = os.listdir(GROUNDTRUTH_DIR)
all_code_file = [i for i in all_code_file
                 if os.path.basename(i) != CACHE_FOLDER_NAME]
...
for i in all_groudtruth_code_name:
    ...
    subp = subprocess.Popen(['gcc', '-O2', '-I', '.', '-o', 
...
for name in all_groudtruth_code_name:
    ...
    # interpret C code
    subp = subprocess.Popen(['./cint', os.path.join(TEST_CASE_DIR, name+INTERPRETED_CODE_EXTENSION)],
...
print('Note that for `jit_robust`, we expect it to fall back!')
